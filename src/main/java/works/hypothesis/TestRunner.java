...
class TestRunner {
    ...
    private void _run() {
        ...
        int mutations = 0;
        int generation = 0;
        while (validExamples <= settings.getMaxExamples() && lastData.getStatus() != Status.INTERESTING && generation <= settings.getMutations()) {
    ...
    void newBuffer() {
        final TestData data = new DistributionalTestData(settings.getBufferSize(), this.random);
        testFunction(data);
        data.freeze();
        lastData = data;
    }
